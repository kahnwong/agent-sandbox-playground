...
async def execute_command(request: ExecuteRequest):
    ...
    try:
        ...
        return ExecuteResponse(
            stdout="", stderr=f"Failed to execute command: {str(e)}", exit_code=1
        )
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"message": f"File upload failed: {str(e)}"}
        )
...
async def download_file(encoded_file_path: str):
    ...
    if os.path.isfile(full_path):
        return FileResponse(
            path=full_path, media_type="application/octet-stream", filename=decoded_path
        )
...
async def list_files(encoded_file_path: str):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"message": f"List files failed: {str(e)}"}
        )
